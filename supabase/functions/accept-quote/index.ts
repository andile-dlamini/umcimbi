...
serve(async (req) => {
  ...
  try {
    ...
    if (serviceReq.requester_user_id !== user.id) {
      ...
      return new Response(JSON.stringify({ error: `Only the client can accept this quote. You are ${user.id} but the requester is ${serviceReq.requester_user_id}` }), { status: 403, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
