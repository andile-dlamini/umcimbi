...
serve(async (req) => {
  ...
  try {
    ...
    // Generate Order Confirmation PDF as a background task — EdgeRuntime.waitUntil
    // keeps the function alive until this fetch settles, so it can't be cut off
    // by the runtime tearing down the context after the response is returned.
